...
cap(sl, 6.85, "ブロック塀のあるT字路・十字路の歩道が定位置。携行機材のみの受動観測は原則許可不要の見込み（三脚・コーン等を使う日は所轄へ事前確認）。協力者がいれば10m後ろ、方位校正も頼める")
...
cap(sl, 0.55, "騒音計の計測中は無言（声が入ると較正が汚れる）。読み値は計測後にひとこと。チェスト装着の方位校正は協力者が必要——ソロの日は三脚固定で校正し、チェストのテイクは協力者のいる日に寄せる")
...
cap(sl, 0.55, "しゃべってよいのは試行と試行の間だけ（試行中は無言）。この一言が正解ラベルの1行になる（複数イベントは行を追加）——フレーム単位のx,y,z・距離の手入力はしない設計")
