# -*- coding: utf-8 -*-
"""v4.3 チューニングval（fold32）のplan生成（2026-09-03、通知v4.3_検討の事前宣言 §1 段階2）。

fold30（第1版）は Phase A で「両分割で同一構成」が不成立＝採用なしとなった
（原因は選定規則の設計ミス。→ 通知v4.2_選定手順の事前宣言 §7.1）。
再選定は min-max 安定規則（§7.4）で行い、そのための**未見データ**として fold32 を作る。
fold30 は勝ち構成の追加検証にのみ使う。

構成は fold30 と同一（既存val(fold2)の写し・1,800本）。違いは
clip_id/split（fold32）と seed 帯（**15G**・全既存plan＋fold30と排他assert）だけ。

使い方:
  PYTHONPATH=scripts:src python scripts/step10_v43tune_plan.py
生成ドライバは本モジュールの v43tune_rows() を直接使う（CSVは監査・員数用の記録）。
"""
...
PLAN_DIR = ROOT / "out" / "dataset_outdoor_siren_v43tune" / "plan"
SEED_BASE = 15_000_000_000      # 既存帯: v42tune=13G / v42tune2=14G / core≈12.42G / conf=12G / eval=9.5G
SEED_STEP = 104_729
N_EXPECT = 1_800


def v43tune_rows() -> list:
    """既存val(fold2)の行を写し、clip_id/split/seedだけ差し替えた1,800行（fold32）。"""
    ...
    for i, src in enumerate(core + ext):
        r = dict(src)
        r["clip_id"] = f"fold32_room1_mix{i + 1:04d}"
        r["split"] = "fold32"
...
def main() -> None:
    ...
    for p in (ROOT / "out").glob("*/plan/*.csv"):
        with open(p, newline="") as f:
            ...
            if rd.fieldnames and "split" in rd.fieldnames:
                assert all(r["split"] != "fold32" for r in rd), f"fold32使用済み: {p}"

    PLAN_DIR.mkdir(parents=True, exist_ok=True)
    fields = sorted({k for r in rows for k in r})
    out_csv = PLAN_DIR / "assignment_v43tune.csv"
    ...
    ledger = ["# v4.2チューニングval第2版(fold32) plan台帳（自動生成）", "",
