...
steps = [
    ("①", "儀式 90秒", "録音開始→手拍子1回（時刻同期）→騒音計LAeqを読み上げ1分（絶対較正）→ベル4方位×1打（方位校正）", AMBER),
...
txt(sl, 0.85, 4.75, 6.0, 0.4, [("撮れ高 計65本（レンタル1週間・実働2日＋注釈半日）",
                                {"size": 13.5, "bold": True})])
chips = [
    ("20", "走行車の統制録音", "速度3段×横2〜15mを散らす"),
    ("20", "弱点検証", "EV5・発進5・自転車5・見通し不良の角5"),
    ("10", "負例", "静穏・繁華街・雨上がり・風（誤警告率）"),
    ("10", "固定・機会", "踏切4＋サイレンは遭遇最優先"),
    ("5", "キックボード", "統制走行"),
]
cx = 0.85
...
txt(sl, 0.85, 2.15, 5.4, 3.3, [
    ...
    ("・方向誤差 中央2.0°、至近距離誤差 0.21m の精密検証はこちらの役割", {}),
    ("・学習もすべて合成（1万200クリップ）", {}),
...
txt(sl, 0.85, 6.05, W - 1.7, 0.7,
    [("フレーム単位の x,y,z・距離を手入力することはない（人間には不可能・設計上も不要）。"
      "注釈は1テイク1行×65行＋時刻合わせで半日の作業量", {"size": 13, "bold": True})])
...
rules = [
    ...
    ("③ 10m頭打ちの教訓", "旧版は出力のtanhで距離が±10mに飽和。監査指摘で「距離だけ線形出力＋log符号化」に修正 → 303m先まで出力でき至近は0.21m精度"),
]
cx = 0.55
...
txt(sl, 0.55, 6.65, W - 1.1, 0.5,
    [("通知層はこの（クラス・方向・距離）を読み、1.5m×2フレーム連続→至近警告／≤3.0m→注意／>3.2m→抑制 に振り分ける",
      {"size": 12, "color": MUTED})])
...
txt(sl, 0.85, 6.3, W - 1.7, 0.7,
    [("残る粒度の課題（正直に）：2台同時のフレームで「1台以上検出」99.9%、「2台として分離」は77.7% ＝ 見逃しは無いが完全分離は今後",
      {"size": 12.5, "bold": True})])
