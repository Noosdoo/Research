# -*- coding: utf-8 -*-
"""v16（v15 の各場面 × 2 つのマイク高さ＝高さ増強）の生成ドライバ。出力先は out/dataset_outdoor_siren_v16/ のみ。

使い方:
  python scripts/_run_v16_gen.py --stats          # 描画せず、critical 層の車の 3D 最接近が 1.5 m → mic_z でどう変わるか
  python scripts/_run_v16_gen.py --rows 0-749     # シャード生成（サーバ）
  python scripts/_run_v16_gen.py --list
"""
...
import step11_v16_render as v15  # noqa: E402
...
def main() -> None:
    ...
    assert len(rows) == 18000, len(rows)
    assert m9.DS.name == "dataset_outdoor_siren_v16", m9.DS
    if "--list" in sys.argv:
