...
def score_clip(rows, gt):
    ...
    n = {"中": 0, "強": 0, "警告": 0, "supp": 0, "refire_raw": 0, "refire_after": 0, "harm": 0, "total_raw": 0}
    for r in rows:
...
def main() -> int:
    ...
    L = [f"# v4.6 車の hold 掃引 — {split}（{len(clips)} 本・モデル予測）", "",
    ...
    for name, C44 in GRID:
        ...
        ok = rr_cut >= 50 and harm <= 2 and reach >= base[2] - 1 and abs(warn - base[3]) < 1e-9
