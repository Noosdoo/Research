# -*- coding: utf-8 -*-
"""fold31（v4.2チューニングval第2版・1,800本）の生成ドライバ。

出力先は **out/dataset_outdoor_siren_v42tune2/**（新規）。既存データセットには書かない。

使い方:
  PYTHONPATH=scripts:src python scripts/_run_v42tune2_gen.py --rows 0-149
  PYTHONPATH=scripts:src python scripts/_run_v42tune2_gen.py --list
...
import step10_v42tune2_plan as plan  # noqa: E402
...
def main() -> None:
    rows = plan.v42tune2_rows()         # m9.DS 付け替え前に呼ぶ
    assert len(rows) == 1800, len(rows)
    m9.DS = ROOT / "out" / "dataset_outdoor_siren_v42tune2"
    m9.WORK = m9.DS / "work"
    if "--list" in sys.argv:
