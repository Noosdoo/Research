# -*- coding: utf-8 -*-
"""v11 run2（IS計算サーバー学習ckpt）の全採点ランナー。

_run_v11_scoring.py（run1用）の run2 パス版。採点ロジックは同一で、
予測= out/predictions_v11run2/、出力= out/step12_notify_v11run2/ に差し替えただけ。
run1 との通知層レベル比較（限定的な頑健性証拠の拡張）が目的。
...
PRED = ROOT / "out" / "predictions_v11run2"
OUT = ROOT / "out" / "step12_notify_v11run2"
CAR = 4
...
def main():
    ...
    rep = ["# v10a 交通量・複数車 採点（v11 run2 server。2026-07-30）",
    ...
    jobs = [
        ("4/6 scn2", [py, str(ROOT / "scripts" / "step15_scn2_score.py"),
                      ...
                      "--pred", "predictions_v11run2/scn2_all.csv",
                      "--out", "step12_notify_v11run2",
                      "--title", "追加5シナリオ 採点（v11 run2 server、ルールv1。2026-07-30）"]),
        ("5/6 halluc", [py, str(ROOT / "scripts" / "step17_halluc_score.py"),
                        "--pred", "out/predictions_v11run2",
                        "--out", "out/step12_notify_v11run2",
                        "--title", "v11 run2 server"]),
