...
step("バックアップ json → 空の別ブラウザに読み込むと同じ CSV が出る（window.claude が無い環境でも動く）", () => { const bk = w.__note.backupJson(); const d2 = boot(null); const w2 = d2.window; const n = w2.__note.importBackup(bk); if (n !== 3) throw new Error("sessions=" + n); if (w2.__note.eventsCsv() !== w.__note.eventsCsv() || w2.__note.sessionCsv() !== w.__note.sessionCsv()) throw new Error("csv differs"); if (!/端末 \d\d:\d\d/.test(w2.document.getElementById("savedAt").textContent)) throw new Error(w2.document.getElementById("savedAt").textContent); });
