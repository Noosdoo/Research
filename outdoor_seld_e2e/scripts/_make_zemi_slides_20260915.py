...
（10月のゼミへ回す）・**金額（2026-09-03本人方針「価格面はなにも載せない」。口頭で話す）**。
...
txt(sl, 508, y0 + 54, 366, 22,
    [[("機種は ", {"size": 12}),
      ("Zoom H3-VR", {"size": 14, "bold": True, "color": RED}),
      ("（アンビソニックスマイク）", {"size": 10.5, "color": SUB})]])
txt(sl, 508, y0 + 84, 366, 40,
    [("9月末〜11月に屋外で計210回の録音を行います。なぜ購入なのかを6〜10枚目でご説明します。",
...
for i, runs in enumerate([
    ...
    [("装着：", {"bold": True}),
     ("鎖骨の間のくぼみにハーネスで固定。全テイク静止で統一", {"color": SUB})],
    [("録音設定：", {"bold": True}),
...
txt(sl, GX + 96, GY + 122, 260, 18,
    [("装着者（鎖骨のくぼみに4方向マイク・静止）", {"size": 10, "color": SUB})],
    align=PP_ALIGN.CENTER)
...
y0 = panel(sl, 70, 256, 400, 122, GOLD, "③ 儀式の中身（90秒・セッション冒頭に1回）")
...
txt(sl, 70, 100, 820, 20,
    [("前の4枚のとおり、天候と人の都合に合わせて、2か月半のあいだ何度も現場に出ることになります。",
      {"size": 12, "color": MUTED})])

reasons = [
    ("1", "測定の条件がそろう",
     "音量の基準合わせは、同じ1台なら最初に1回やれば210回すべてに通ります。"
     "借りると毎回ちがう個体になり、やり直すたびに測定条件が変わります。"),
    ("2", "天候と機会に合わせて、いつでも出られる",
     "雨上がりの日も、風の日も、協力者の空いた日も、手元にあれば出られます。"
     "借りている期間に合わせて天気を待つことはできません。"),
    ("3", "修士の測定の基準器になる",
...
for i, (no, head, body) in enumerate(reasons):
    ...
    txt(sl, 114, y + 31, 450, 42, [(body, {"size": 10.5, "color": SUB})], line=1.35)

y0 = panel(sl, 598, 132, 292, 254, PUR, "なぜこの機種なのか")
txt(sl, 616, y0, 256, 48,
    [("上下も含めた4方向を同時に録れる機種は、ZOOMの現行品では2つだけです。",
      {"size": 10.5, "color": SUB})], line=1.35)

for i, (nm, wt, note, col) in enumerate([
    ("H3-VR", "166g", "これ。4方向で録れる機種で最軽量", GREEN),
    ("H8＋専用カプセル", "576g＋", "同じことはできるが、身に着けるには重い", MUTED),
]):
    yy = y0 + 54 + i * 52
    rect(sl, 616, yy, 256, 44, PALE if i == 0 else BAND)
    txt(sl, 628, yy + 5, 170, 18, [(nm, {"size": 11.5, "bold": True, "color": col})])
    txt(sl, 790, yy + 5, 70, 18,
        [(wt, {"size": 11, "bold": True, "color": col})], align=PP_ALIGN.RIGHT)
    txt(sl, 628, yy + 24, 232, 16, [(note, {"size": 9, "color": MUTED})])

txt(sl, 616, y0 + 162, 256, 56,
    [[("ほかの機種は全部ステレオ方式で、", {"size": 10.5, "color": SUB})],
     [("高さの軸がありません。", {"size": 10.5, "bold": True, "color": RED}),
      ("学習に使ったデータと形式が合わず、モデルに入力できません。",
       {"size": 10.5, "color": SUB})]], line=1.35)

rect(sl, 70, 398, 820, 60, PALEG)
rect(sl, 70, 398, 4, 60, GOLD)
txt(sl, 88, 406, 790, 46,
