...
# ============ 10 検証2 ============
sl = new_slide()
header(sl, "RESULTS 2/2", "検証 — 人の耳より先に気づけるか")
fx, fy = 54, 108
...
label(sl, fx + 12, fy + 122, W - 132,
      "サイレン初検知距離（100〜300mの遠方配置・未検知ゼロ）—「人が気づく前から機械の耳は聞いている」を距離で定量化",
...
items = [
    ...
    ("通知", "至近警告の到達は約7割。未達の主因は距離推定が2フレーム連続で1.5mに届かないケース（失敗例は全数分類済み・安全な車への誤帰属はほぼ無し）"),
...
bullets(sl, 54, 120, W - 108, [
    ...
    {"runs": [("「聞こえる前」を定量化 — サイレン初検知 ", {}),
              ...
              ("・40kHz超音波検出も初実装", {})]},
    {"runs": [("次：", {}), ("実環境録音での評価", {"bold": True, "color": INK}),
              ("と", {}),
              ("物理要素の切り分け実験", {"bold": True, "color": INK}),
              ("で「どの物理が必要か」に答える", {})]},
], size=16, gap=20)
stripe(sl, 456)
footer(sl, 5)
