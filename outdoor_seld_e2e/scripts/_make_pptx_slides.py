# -*- coding: utf-8 -*-
"""中間発表スライドの.pptx生成（HTML版=中間発表スライド_2026-08-10.htmlと同内容）。
...
bullets(sl, 54, 120, 440, [
    ...
    {"mcolor": AMBER,
     "runs": [("機械の耳は人間の耳の限界（可聴域・可聴距離）に縛られない\n→「耳の代わり」を超えて", {}),
              ("人の耳を超える支援", {"bold": True, "color": AMBERD}),
              ("まで狙える", {})]},
])
fx, fy = 560, 112   # 図の原点
...
tf.margin_top = Pt(10)
para(tf, "8/4からの拡張", size=14, bold=True, color=INK, after=4, first=True)
para(tf, [("クラス：6 → ", {}), ("8", {"bold": True, "color": INK}),
          ("（キックボード・バイク・列車を追加）　／　推定量：種類＋方向 → ", {}),
          ("＋距離", {"bold": True, "color": AMBERD}),
          ("　／　音源仕様：", {}),
...
for i, t in enumerate(["伝搬遅延", "ドップラー", "距離減衰", "大気吸収"]):
    b = rect(sl, fx + 96 + i * 62, fy + 66 + i * 16, 58, 20, fill=WHITE, line=HAIR)
...
para(tf, [("歩行者装着 × 複数危険クラス同時 × 距離まで", {"bold": True, "color": INK}),
          ("を屋外で扱った研究は存在しない。分業型（分類CNN＋信号処理DOA）は複数音源の同時発生で対応付けが壊れる → 同時推定型SELDを採用",
...
for i, (t, body) in enumerate([
    ...
    ("4ch音＋方向・距離ラベル", "同一経路から同時に出力（FOA）。ラベル誤差ゼロの「教科書」"),
    ("学習・評価", "PSELDNetsをファインチューニング。学習10,200＋確定評価1,800クリップ"),
]):
...
bullets(sl, 54, 268, W - 108, [
    {"runs": [("なぜ合成か：① 屋外の方向つきデータが他に存在しない　② 物理を自分で持てば1つずつ外せる＝", {}),
...
c = card(sl, x0, 194, cw2, 118, title="通知層（3役割）— 推定距離で出し分け", border=AMBER,
         lw=1.5)
para(c.text_frame, [("至近警告", {"bold": True, "color": RED}),
                    ("　推定距離≤1.5mが2フレーム連続（同一物体の方位連続性つき）", {})],
...
para(tb.text_frame, "8/4時点は危険度を区別できなかった → 距離推定の統合で3段階が機能（今回の最大の進捗）",
...
# ============ 9 検証1 ============
sl = new_slide()
header(sl, "RESULTS 1/2 — 確定評価", "検証 — 事前固定した未使用セットでの一発評価")
tb = box(sl, 54, 100, W - 108, 44)
para(tb.text_frame,
     [("作法：合格基準を", {}), ("先に宣言", {"bold": True, "color": INK}),
      (" → ", {}), ("未使用の評価セット1,800本", {"bold": True, "color": INK}),
      ("を新規生成 → ", {}), ("1回だけ", {"bold": True, "color": INK}),
      ("推論・採点。AI外部監査（敵対的レビュー3巡）で", {}),
      ("完全合格", {"bold": True, "color": AMBERD})],
     size=13.5, color=INK2, line=1.3, after=0, first=True)
rows = [
    ("ゲート", "事前宣言した基準", "確定値", "判定"),
    ("G1 基本", "車 ≥99%・方向 ≤3.0°", "99.4% ／ 2.0°", "達成"),
    ("G2 静音EV", "単独EV ≥95%", "99.0%（前世代80.2%から+19pt）", "達成"),
    ("G3 新クラス", "バイク≥90／踏切列車≥99／キック≥60", "99.8 ／ 100.0 ／ 97.3%", "達成"),
    ("G4 距離", "至近中央 ≤0.30m・重大判定 ≥68%", "0.21m ／ 69.0%", "達成"),
    ("G5 総合", "SELDスコア ≤0.085", "0.087（学習時指標）", "未達"),
]
...
para(tb.text_frame, "8クラス可聴検出率 97.2〜100%。未達も隠さず記載するのが本研究の検証の作法。",
     size=12, color=MUTED, after=0, first=True)
footer(sl, 4)

# ============ 10 検証2 ============
sl = new_slide()
header(sl, "RESULTS 2/2 — 聞こえる前", "検証 — 人の耳より先に気づけるか")
fx, fy = 54, 108
...
label(sl, fx + 12, fy + 122, W - 132,
      "サイレン初検知距離（100〜300m遠方配置・未検知ゼロ・確定評価）—「人が気づく前から機械の耳は聞いている」を距離で定量化",
      size=10.5, align=PP_ALIGN.LEFT)
cw = (W - 108 - 40) / 3
for i, (t, lines) in enumerate([
    ("通知層（確定評価）", [[("重大車への至近警告到達 ", {}), ("69.9%", {"bold": True, "color": INK, "size": 15})],
...
for i, (t, body) in enumerate([
    ...
    ("測り方", "各版を全物理の確定評価セット＋実録60本の共通土俵で採点 → 落ちた分＝その物理の大事さ"),
    ("準備状況", "学習レシピ・採点系・評価セットは確定済み（計画第5版）。9月に実録と同じ土俵で実施"),
]):
...
for i, (tag, col, body) in enumerate([
    ("① 統制録音", INK2, "車（速度×横距離を散らす）・EV・キックボード・踏切＋列車。台本どおり幾何を合成シナリオに揃える"),
    ("② 機会捕捉", AMBERD, "サイレン・クラクション・バック音。遭遇したら他を中断して最優先記録"),
    ("③ 負例", GREEN, "静穏・繁華街・風。誤警告率／時の実世界試験"),
]):
...
items = [
    ("実録", "実環境は未検証（9月に測定）。合成の空間手がかりは理想的過ぎる可能性"),
    ("指標", "総合SELDスコアは事前基準に僅差未達（0.087 vs 0.085。個別性能は全達成）"),
    ("通知", "至近警告の到達は約7割。未達の主因は距離推定が2フレーム連続で1.5mに届かないケース（個票監査で全数分類済み・安全車への誤帰属はほぼ無し）"),
    ("学習", "キックボードの高騒音下検出は88.7% — 距離重視の学習とのトレードオフが残る"),
    ("設計", "通知の同一物体判定は方位連結による近似（完全なトラッキングは将来課題）"),
]
y = 116
for tag, body in items:
    tagchip(sl, 54, y + 2, tag, INK2)
    tb = box(sl, 116, y - 4, W - 170, 60)
    para(tb.text_frame, body, size=13.5, color=INK2, line=1.3, after=0, first=True)
    y += 62
tb = box(sl, 54, y + 6, W - 108, 30)
para(tb.text_frame, "限界も「どのケースが・何台・なぜ」まで個票レベルで分解できているのが現在の状態。",
     size=12, color=MUTED, after=0, first=True)
footer(sl, 5)
...
bullets(sl, 54, 120, W - 108, [
    {"runs": [("8クラス・", {}), ("距離つき", {"bold": True, "color": INK}),
              ...
              ("事前固定の未使用セットで確定評価", {"bold": True, "color": INK}),
              ("（ゲート4/5達成・AI外部監査 完全合格）", {})]},
    ...
    {"runs": [("「聞こえる前」を定量化 — サイレン初検知 ", {}),
              ("208m", {"bold": True, "color": INK, "size": 30}),
              ("・40kHz超音波検出も初実装", {})]},
    {"runs": [("次：", {}), ("9月の実録評価", {"bold": True, "color": INK}),
              ("（測定方法確定済み）と", {}),
              ("物理ablation", {"bold": True, "color": INK}),
              ("で「どの物理が必要か」に答える", {})]},
], size=16, gap=20)
stripe(sl, 456)
footer(sl, 5)
...
# ============ 17 付録B ============
sl = new_slide()
header(sl, "APPENDIX B", "付録 — 確定評価と外部監査の仕組み")
bullets(sl, 54, 120, W - 108, [
    {"runs": [("確定評価セット", {"bold": True, "color": INK}),
              ("：検証セットと同一設計・新乱数・未使用フォールドの1,800本。", {}),
              ("生成前に計画をコミット", {"bold": True, "color": INK}),
              ("（事前登録）し、推論・採点は1回だけ", {})]},
    {"runs": [("開発値と確定値の乖離は全指標±2pt以内 ＝ 過学習ではなく汎化している直接証拠", {})]},
    {"runs": [("AI外部監査", {"bold": True, "color": INK}),
              ("：別のAI（監査役）による敵対的レビュー。不合格指摘 → 全対応 → 再検証3巡 → 完全合格。指摘には通知ルールの欠陥発見（同一物体の連続性）も含まれ、ルール改訂の契機になった", {})]},
