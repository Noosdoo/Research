...
items = [
    ("実録", "実環境は未検証。9月に120本（統制100＋静止/歩行対比20）で測るが、"
             "測れるのは「胸部装着・実環境での通知性能」まで"),
...
bullets(sl, 54, 120, W - 108, [
    ...
    {"runs": [("次：9月の", {}), ("実環境120本", {"bold": True, "color": INK}),
              ("を全ablation版の", {}),
              ("共通試験", {"bold": True, "color": INK}),
              ("にして、「どの物理が必要か」と「実環境でどれだけ落ちるか」を同じ土俵で測る", {})]},
], size=16, gap=20)
stripe(sl, 456)
footer(sl, 5)
