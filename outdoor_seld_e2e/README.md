...
## 次の一手（2026-07-16更新。設計の正= out/v9_design_v2_2026-07-16.md）

1. **v9の実装と生成**（最終土俵。割当表→絶対較正ミキサ→検品v9対応→生成前チェック→
   スモーク→全量1120本。v2設計書11節の順）
2. 通知層スクリプト（step11_notify.py: ルールv1＋リードタイム採点＋オラクル上限）
3. v9学習（Colab）＋レベル正規化プローブ評価 → 基盤完成宣言 → ゼミ発表
4. ablation計画第4版（v9基準）→ ablation 5本
5. 8月: 実録スモーク評価・環境実測・文献宿題（goal_definition冒頭の宿題リスト参照）

（旧記載のシーン量産・物理スイッチは実装済み。FSD50K差し替えは不採用=クリーン合成方針）
