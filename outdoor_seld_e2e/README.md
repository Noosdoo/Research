...
- **完了済み**: v9.2 の生成・学習・評価（複数車 0→58.7% ほか、正= md/results/v9.2最終結果_2026-07-19.md）、
  因果推論によるリアルタイム性検証（md/audit/因果推論リアルタイム性検証_2026-07-19.md）。
- **第5回監査（外部）対応**: 評価層の修正（通知時刻+0.1s補正・誤通知率のPoisson上限・
  Wilson区間・分母是正）を実装済み。正= **md/audit/第5回監査への対応_2026-07-19.md**。

1. **ゼミ8/4の資料づくり**（骨子= md/seminar/ゼミ発表骨子_20260804.md、別途進行中）
2. **要Colab**: fold3での最終テスト評価（現数値は開発性能）、batch=1の実時間ベンチ、
   車イベント数を揃えた対照の再学習（audit5 の残タスク5節）
3. 8月: 実録スモーク収録・評価＋環境実測・文献宿題
4. 8/4後: ablation 5本（md/design/アブレーション実験計画_2026-07.md 8節。通知採点= scripts/step12_notify_v9.py）
