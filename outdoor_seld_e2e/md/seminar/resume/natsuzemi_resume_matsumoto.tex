...
\title{\vspace{-8mm}\bf SELDモデルを用いた難聴歩行者向け屋外危険音通知システムの構築}
\author{松本 鋭\thanks{東京理科大学 創域理工学部 情報計算科学科 4年}\\ Satoshi Matsumoto}
\date{}
...
\twocolumn[
  \begin{@twocolumnfalse}
    \hfill{\small 2026/09/XX 夏ゼミ配布資料}\\[-2mm]
    \maketitle
    \vspace{-6mm}
  \end{@twocolumnfalse}
]
...
\subsection{実時間動作}
上記は判定時刻より後の音も参照した値である．
実機を想定し，各時刻までの音のみで判定する因果推論に切り替えると，
至近到達は 50.2\% へ低下した．
原因は情報の欠落ではなく推定値の偏りであり，
至近と安全を距離で分離する能力は AUC 0.996 から 0.987 とほぼ保たれていた．
そこで検証データのみで作成した単調変換で補正したところ
\textbf{62.8\%} まで回復した．
現時点では，未来を参照していた従来方式の 69.9\% に近い水準を，
未来を参照せずに達成できている．
