# -*- coding: utf-8 -*-
"""v17（装着高さ入力）SDE学習の起動ラッパ = v12 版 ＋ height_patch（**因果窓版**）= 空メタ許容 + sde_patch + causal_patch + train。

学習時の入力を推論時とまったく同じ「右詰め・ゼロ埋めの10秒窓」にし、
最後の1フレームだけを教師にする。詳細は causal_patch.py。
検証・評価は通常どおり（比較の土俵を保つため）。
