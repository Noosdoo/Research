...
## ⚡ いま生きている結論の一覧（2026-07-16時点）

本文は時系列記録のため、古い結論が訂正注記付きで残っている。読み違え防止のため**この表が最新の正**。

| テーマ | 生きている結論 | 死んだ/訂正された結論 |
| --- | --- | --- |
| **v9設計** | **改訂第2版が唯一の正**（out/v9_design_v2_2026-07-16.md。6クラス＝踏切追加・1120本・絶対較正・歩行混在・通知層込み）。**07-17: 生成完了・検品FAIL 0/1188**（PROGRESS「v9生成完了」節） | ~~±12dBジッタ／車SNR軸／960本／「歩行はv10」~~ |
| 音量ショートカット(D1) | **案A''で決着**: 法規レンジの絶対較正＝音量は現実の正当な手がかり。音色識別の証明はレベル正規化プローブセットで行う | ~~案A（±12dB一律ジッタ）~~ |
| 車クラス | **パイロット合格**（substitution 0・車LE 2.24°・重なり同時検出85.9%・負例で幻覚0）→ v9採用確定 | — |
| 歩行マイク | エンジン拡張済み・検証5件PASS（静止一致・観測者ドップラー・DOA解析解・二分法照合・既存経路ビット一致） | ~~v10送り~~ |
| 通知層 | **卒論スコープで実装**（ルールv1＋リードタイム採点＋オラクル上限。定義=notification_design_research） | ~~将来課題~~ |
...
| スコアの絶対値 | 文献と比較不可（簡単な土俵）。主張は相対比較・解剖指標・通知指標で | — |
| 実験の土俵 | 学習済みの現行土俵はv8。**最終土俵はv9**（設計確定・未生成）。v5-v7の数値との直接比較禁止 | ~~v7がablation基準~~ ~~v8が最終~~ |
| 当事者指標 | イベント見逃し0%・誤通知1.35/分・初動方向30-80%（step8dで測定可能） | ~~初動方向35-80%~~（転記ミス訂正） |
...
- 検品: **全1348本 FAIL 0**（検品側の2バグを修正: ①複数車はトラック番号で自分の
...
- **発見①: 同一クラス多重は学習分布外** — v9.1学習モデルは複数車を同時報告できず
  ...
  15°超で分離できる実装と確認済み→データ由来。対策=複数車学習は凍結解除事項（8/4相談）
