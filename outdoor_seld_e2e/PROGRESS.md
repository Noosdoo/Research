...
| **v9.2（決着）** | **学習追加180本で多重車の同時検出 0%→58.7%（2台両方）/70.5%（3台で2+）、対照ctrl（増量のみ180本）は全指標0%のまま=効いたのは多重の教師例と確定**。車なし負例50本で幻覚の持続が崩壊（14.8→1.5フレーム/本）→車の誤通知発火0/50件。val240/probe48/6シナリオは劣化なし（S4誤報0維持）。正= out/v9_2_results_2026-07-19.md | ~~「再学習せず限界記載」方針~~ ~~「事前登録#1の2台両方≥70%」（58.7%で未達=量の知見）~~ |
...
| **リアルタイム性** | **因果（過去のみ）推論で決着**: 通知率は6シナリオ全維持（S2はむしろ+1本）・誤報0のまま・初通知の遅れ中央値+0.0〜0.25s（最悪+1.2s）・1判定11.3ms(T4)=実時間の1/9。遅延予算=警告音0.4〜0.6s/車1.1〜1.3s。正= out/causal_realtime_2026-07-19.md | ~~「未来参照で楽観のまま未検証」~~ |
