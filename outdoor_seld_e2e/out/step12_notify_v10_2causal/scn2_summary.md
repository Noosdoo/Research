# 追加5シナリオ 採点（v10.2 因果推論=過去のみ、2026-07-23）
...
- 通知: 20/20 (100% [95%CI 84–100])、発火遅れ中央値 0.39s、**追い越しの 3.0s 前**（中央値）に通知
- 通知方向誤差 中央値 3.0°（ほぼ真後ろの音源＝視覚の効かない方向）

## S3 駐車場のバック車（バック音=車と同一軌道 20本）
- critical: 通知 9/10 (90% [95%CI 60–98])、リードタイム中央値 5.2s / >=2.5s 89%
- caution: 通知 10/10 (100% [95%CI 72–100])、リードタイム中央値 6.0s / >=2.5s 100%
- バック音（beep）の通知方向と車真方向の差 中央値 5.8°（=警告音が車の方向を正しく代弁）

## S5 悪条件サイレン（暗騒音60-65dB固定×遠方10-15m 20本）
- 通知: 20/20 (100% [95%CI 84–100])、リードタイム中央値 6.9s / >=2.5s 100%
- オラクル上限: 中央値 7.2s（差=モデル+ルールの遅れ）
- 初通知時の距離 中央値 79m、通知方向誤差 中央値 3.2°
