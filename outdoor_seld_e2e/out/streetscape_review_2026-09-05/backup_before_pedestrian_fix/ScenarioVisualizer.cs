...
public class ScenarioVisualizer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        pedestrian.transform.position = new Vector3(0, 1.12f, 0);
    ...
    void UpdateCamera()
    {
        ...
        if (cameraMode == 1)
        {
            cam.transform.position = new Vector3(0, 1.68f, -.25f);
    ...
    void Update()
    {
        ...
        if (Mathf.Abs(scroll) > .01f) { cameraZoom = Mathf.Clamp(cameraZoom - scroll * .08f, .45f, 1.8f); UpdateCamera(); }
