...
public sealed class JapaneseStreetscape : MonoBehaviour
{
    ...
    void Generate(string path)
    {
        ...
        else
        {
            ...
            if (roads.FindAll(r=>!r.rail).Count == 0)
                roads.Add(new Road { a=new Vector3(3,0,-Extent),b=new Vector3(3,0,Extent),width=4.8f });
        }
    ...
    void AddLaneRoads(List<Lane> input, bool horizontal)
    {
        ...
        for(int i=0;i<cars.Count;)
        {
            ...
            while(j<cars.Count && cars[j].at-max<7.1f) { max=cars[j].at; j++; }
    ...
    void NeighborhoodRoads()
    {
        ...
        bool observerHasPavement=longitudinal.Exists(r=>Mathf.Abs(r.a.x)<r.width*.5f+1.8f);
        if(!observerHasPavement)
            roads.Add(new Road{a=new Vector3(0,0,-Extent),b=new Vector3(0,0,Extent),width=2.2f,foot=true});
    ...
    bool Blocked(Vector3 p,float radius)
    {
        ...
        if(!MapMode && Distance(p,new Vector3(0,0,-90),new Vector3(0,0,90))<radius+1.2f)return true;
    ...
    void DrawRoad(Road r)
    {
        ...
        else Strip("asphalt",r.a,r.b,r.width,.005f,false);
        // Round caps close angled map segments without a stack of square road ends.
        if(MapMode) foreach(Vector3 at in new[]{r.a,r.b}) Cylinder(r.foot?"pavers":"asphalt",at+Vector3.up*(r.foot?.07f:-.014f),new Vector3(r.width,.04f,r.width),false);
        if(r.foot) return;
        ...
        for(float s=1;s<len-1;s+=1.5f)
        {
            ...
            for(int side=-1;side<=1;side+=2)
            {
                ...
                if(Junction(edge,r,1.2f)) continue;
                if(urban)
                {
