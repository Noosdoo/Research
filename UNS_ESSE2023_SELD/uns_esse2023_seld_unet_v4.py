"""
SELD U-Net for UNS-ESSE2023  (v4)
===================================
v3 からの変更点:
  - Phase 2 損失: 活性フレーム重み付き ADPIT → BCE(検出) + MSE(定位) 分離損失
  - n_tracks: 3 → 1 (UNS-ESSE2023 は 1 イベント/clip、multi-ACCDOA 不要)
  - BCE pos_weight=9 でクラス不均衡（活性 10%）を補正
...
━━━ 元論文との相違点（変更が必要な箇所） ━━━━━━━━━━━━━━━━━━
  ...
  ✗ Phase 2 損失: ADPIT → BCE(検出)+MSE(定位) の分離損失 + pos_weight=9
  ✗ n_tracks: 3 (論文) → 1 (UNS-ESSE2023 は同時音源なし)
  [出力]
