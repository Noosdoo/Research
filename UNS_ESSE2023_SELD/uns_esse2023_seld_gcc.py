"""
UNS-ESSE2023 SELD U-Net 評価スクリプト (GCC-PHAT 空間特徴量版)
...
【uns_esse2023_seld.py からの変更点】
  - 特徴量: 8ch mel + 7ch GCC-PHAT = 15ch (n_channels=15)
  - GCC-PHAT: マイク0 (基準) vs マイク1〜7 の7ペア
  - n_tracks=3 は SELD U-Net 論文準拠で維持

【GCC-PHAT とは】
  Generalized Cross-Correlation with Phase Transform。
  マイクペア間の音の到来時間差 (TDOA) を周波数領域で計算する。
  8ch マイクアレイにおいて方向情報を明示的に特徴量化する。
...
【動作モード】
  python uns_esse2023_seld_gcc.py --mode train
  python uns_esse2023_seld_gcc.py --mode eval
  python uns_esse2023_seld_gcc.py --mode demo
"""
...
CFG = {
    ...
    "output_dir":     "./output_gcc",      # 元の output と分離
    "train_split":    "train",
    "eval_split":     "eval",
    ...
    "hop_length":     512,
    "n_mels":         64,
    "n_channels":     15,   # 8 mel + 7 GCC-PHAT (mic0 vs mic1〜7)
...
print(f"n_channels={CFG['n_channels']} (8 mel + 7 GCC-PHAT)")
print(f"n_frames={CFG['n_frames']}, label_frames={CFG['label_frames']}")

# =============================================================================
# 2. 特徴量抽出 (8ch MIC → 8 mel + 7 GCC-PHAT = 15ch)
# =============================================================================

def _load_audio_8ch(audio_path, target_fs):
    """WAV を読み込み、8ch にパディング・リサンプルして返す。(8, samples)"""
    ...
    if sr != target_fs:
        audio = np.stack([
            librosa.resample(audio[c], orig_sr=sr, target_sr=target_fs)
            for c in range(8)
        ])
    return audio.astype(np.float32)  # (8, samples)


def _mel_spec(signal, n_fft, hop_length, mel_basis):
    """メルスペクトログラム (dB スケール): (n_mels, T)"""
    stft  = librosa.stft(signal, n_fft=n_fft, hop_length=hop_length,
                         win_length=n_fft, window="hann")
    power = np.abs(stft) ** 2
    return librosa.power_to_db(mel_basis @ power, ref=np.max), stft


def _gcc_phat_mel(stft_ref, stft_other, mel_basis):
    """
    mel スケール GCC-PHAT。

    GCC-PHAT の周波数領域表現: R = X_ref * conj(X_other) / |X_ref * conj(X_other)|
    この実部をメルフィルタバンクに通すことで、マイクペア間の位相関係を
    mel スケールで表現する。音の到来方向に応じて値が変化する空間特徴量。

    Returns: (n_mels, T)
    """
    R = stft_ref * np.conj(stft_other)
    R_norm = R / (np.abs(R) + 1e-8)        # PHAT 正規化
    gcc = np.real(R_norm)                   # 実部: (n_fft//2+1, T)
    return mel_basis @ gcc                  # (n_mels, T)


def extract_mic_gcc_features(audio_path, cfg=CFG):
    """
    8ch マイクアレイ WAV から 15ch 特徴量を抽出する。
      - 8ch メルスペクトログラム (振幅情報)
      - 7ch GCC-PHAT (マイク0 基準, mic0 vs mic1〜7) (空間情報)

    Returns:
        feat (np.ndarray): shape (15, T, n_mels) - float32
    """
    audio      = _load_audio_8ch(audio_path, cfg["fs"])
    n_fft      = cfg["n_fft"]
    hop_length = cfg["hop_length"]
    mel_basis  = librosa.filters.mel(sr=cfg["fs"], n_fft=n_fft, n_mels=cfg["n_mels"])

    # 各チャンネルの mel + STFT を取得
    mel_feats = []
    stfts     = []
    for c in range(8):
        mel, stft = _mel_spec(audio[c], n_fft, hop_length, mel_basis)
        mel_feats.append(mel)
        stfts.append(stft)

    # GCC-PHAT: マイク0 (基準) vs マイク1〜7
    gcc_feats = []
    for c in range(1, 8):
        gcc = _gcc_phat_mel(stfts[0], stfts[c], mel_basis)
        gcc_feats.append(gcc)

    # (15, n_mels, T) → (15, T, n_mels)
    feat = np.stack(mel_feats + gcc_feats, axis=0)  # (15, n_mels, T)
    feat = feat.transpose(0, 2, 1)                   # (15, T, n_mels)
    ...
    def __init__(self, cfg=CFG, split="train", augment=False,
        ...
        for audio_path, events in file_events.items():
            if not os.path.exists(audio_path):
                ...
                continue
            feat = normalize_feature(extract_mic_gcc_features(audio_path, cfg))
...
class UNet(nn.Module):
    """入力: (B, 15, T, n_mels) → 出力: (B, 15, T, n_mels)"""
    def __init__(self, in_ch=15, base_ch=32):
        super().__init__()
        b = base_ch
...
class SELDMetrics:
    """
    DCASE2023 準拠: ER, F1 (location-dependent) + LE_CD, LR_CD (class-dependent)
    ...
    def reset(self):
        ...
        self.cls_loc_err = [0.0] * self.C
        self.cls_loc_tp  = [0]   * self.C
        self.cls_ref     = [0]   * self.C
        # ハンガリアン法用（LE_CD / LR_CD）
        self.hung_err   = [0.0] * self.C
    ...
    def update(self, pred_events, ref_events, n_frames):
        ...
        for key in set(pred_d) | set(ref_d):
            ...
            # ハンガリアン法で LE_CD / LR_CD 用マッチング（閾値なし）
            self.hung_ref[cls] += len(refs)
            if preds and refs:
                D = np.array([[angular_distance(p[0], p[1], r[0], r[1])
                                for r in refs] for p in preds])
    ...
    def compute(self):
        ...
        ER = max(self.FP, self.FN) / (self.TP + self.FN + 1e-8)
        # LE_CD（ハンガリアン法・閾値なし）
        le_per_cls = [self.hung_err[c] / self.hung_match[c]
                      for c in range(self.C) if self.hung_match[c] > 0]
        LE_CD = sum(le_per_cls) / len(le_per_cls) if le_per_cls else 0.0
        # LR_CD（ハンガリアン法・閾値なし）
        lr_per_cls = [self.hung_match[c] / self.hung_ref[c]
                      if self.hung_ref[c] > 0 else 0.0
...
def train(cfg=CFG):
    ...
    print("\n=== Phase 1: U-Net 学習 ===")
    opt1, best1, h1 = torch.optim.Adam(model.unet.parameters(), lr=cfg["lr"]), float("inf"), []
...
def evaluate(cfg=CFG):
    ...
    print("\n=== SELD 評価結果 (UNS-ESSE2023, GCC-PHAT) ===")
    if any(r[2] for segs in results.values() for r in segs):
        res = metrics.compute()
        print(f"  Error Rate     (ER)  : {res['ER']:.4f}   (↓)")
        print(f"  F-score        (F1)  : {res['F1']:.4f}   (↑)")
        print(f"  LE_CD               : {res['LE_CD']:.2f} deg (↓)")
        print(f"  LR_CD               : {res['LR_CD']:.4f}   (↑)")
        print(f"  SELD Score          : {res['SELD']:.4f}   (↓)")
...
def generate_sample_data(cfg=CFG, n_files=6, duration=10.0):
    ...
    class_params = {
        0: {"freq": 80,   "dur": (0.3, 1.5), "amp": 0.5},
        1: {"freq": 200,  "dur": (0.05, 0.3), "amp": 0.6},
        2: {"freq": 3000, "dur": (0.2, 0.8), "amp": 0.3},
    }
    ...
    fieldnames = ["filename","event_onset","event_offset","event_label",
                  "event_azimuth","event_elevation","event_distance",
    ...
    print(f"サンプルデータ生成完了: {n_files} ファイル")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SELD U-Net + GCC-PHAT for UNS-ESSE2023")
    ...
    if args.mode == "demo":
        print("=" * 60)
        print("DEMO: GCC-PHAT 版 動作確認")
        print("=" * 60)
        CFG["batch_size"] = 2
        if not args.epochs_p1: CFG["epochs_phase1"] = 5
        if not args.epochs_p2: CFG["epochs_phase2"] = 5
        generate_sample_data(CFG, n_files=9)
        train(CFG)
        evaluate(CFG)

    elif args.mode == "train":
        print("=" * 60)
        print(f"学習 (GCC-PHAT) | split={CFG['train_split']}")
        print("=" * 60)
        train(CFG)

    elif args.mode == "eval":
        print("=" * 60)
        print(f"評価 (GCC-PHAT) | split={CFG['eval_split']}")
        print("=" * 60)
        evaluate(CFG)
