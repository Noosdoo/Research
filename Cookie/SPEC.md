...
- **獲得ポイントはサーバーが正規値を引く**。クライアントは `earnedPoints` を送ってくるが信用せず、サーバーが `lib/siteScores.json`（id→points の正本）から `siteId` に対応する正規ポイントを引いて加算する（クライアントが任意の点数を申告する不正を封じる）
- **`siteId` は実在検証する**。クライアント申告の `siteId` は正規42サイトに存在するものだけ受理し、存在しないIDで勝手にサイト状態を作らせない（`getSiteState`）
- `server.js` はプレーンJSで TypeScript の `lib/sites.ts` を直接 import できないため、点数の正本は両者が読める `lib/siteScores.json` に置く。`sites.ts` は起動時（dev）に各サイトの `points` と JSON の一致を検証し、片方だけ変更したドリフトを即検知する
- サイト状態はサーバー側で遅延生成（訪問されて初めて作る）ので、サイト追加時の `server.js` 改修は不要（点数を足すときだけ `siteScores.json` も更新する）
